...
== Problem Statement
Despite the high amount of research that evaluates LLM-generated code, most of it performs this evaluation in terms of its correctness. This is, of course, a necessity when it comes to deciding
whether or not to start using LLMs when producing software. However, we also need to look at how energy efficient a correct solution generated by an LLM is, and to see if LLMs are able to give
us correct solutions that not only are ready to be released into production, but are also meeting the criteria for an energy-efficient solution.
